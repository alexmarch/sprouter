...
var SpRouter = module.exports = function SpRouter (app, options) {
    this._app = app || {},
        this._options = options || {auto: true, watch: true},
        this.baseDir = '',
        this.routers = {};
    if (this.hasProp(this._options, 'auto') != false) {
        this.run();
    }
    if (this.hasProp(this._options, 'watch') != false) {
        this.watch();
    }
...
SpRouter.prototype = {
                ...
                if (!err && stat.isDirectory()) {
                    fs.readdir(self.baseDir, function (err, files) {
                        if (!err && files.length > 0) {
                            ...
                            console.warn("Path with routers is empty !");
                        }
                    });
                }else{
                    console.warn('[SpRouter] :' + err);
                }
            });
        }
    },
    ...
    include: function (routes_path) {
        try {
            ...
            if(Object.keys(router).length == 0) { console.warn('SpRouter [WARN]:',router); return };
            ...
            console.warn("[SpRouter] Error load router", e);
        }
    },
